...
class WebCallOptions extends CallOptions {
  ...
  WebCallOptions._(Map<String, String> metadata, Duration? timeout, List<MetadataProvider> metadataProviders, {this.bypassCorsPreflight, this.withCredentials})
  ...
  factory WebCallOptions({Map<String, String>? metadata, Duration? timeout, List<MetadataProvider>? providers, bool? bypassCorsPreflight, bool? withCredentials}) {
  ...
  CallOptions mergedWith(CallOptions? other) {
    ...
    if (other is! WebCallOptions) {
      return WebCallOptions._(Map.unmodifiable(mergedMetadata), mergedTimeout, List.unmodifiable(mergedProviders), bypassCorsPreflight: bypassCorsPreflight, withCredentials: withCredentials);
    }
...
// TODO(sigurdm): Find out why we do this.
  ...
  void onConnectionReady(ClientConnection connection) {
    ...
    if (options.metadataProviders.isEmpty) {
      ...
      Future.forEach(options.metadataProviders, (MetadataProvider provider) => provider(metadata, '${connection.scheme}://${connection.authority}${audiencePath(_method)}'))
  ...
  void _sendRequest(ClientConnection connection, Map<String, String> metadata) {
    ...
    _requestSubscription = _requests
        ...
        .listen(stream.outgoingMessages.add, onError: stream.outgoingMessages.addError, onDone: stream.outgoingMessages.close, cancelOnError: true);
    _stream = stream;
  ...
  void _onTimedOut() {
    const error = GrpcError.deadlineExceeded('Deadline exceeded');
    _finishTimelineWithError(error, _requestTimeline);
    _responses.addErrorIfNotClosed(error);
    _safeTerminate();
  }
  ...
  void _onResponseListen() {
    if (_stream != null && _responses.hasListener && _responseSubscription == null) {
      ...
      final subscription = _stream!.incomingMessages.listen(_onResponseData, onError: _onResponseError, onDone: _onResponseDone, cancelOnError: true);
  ...
  void _onResponseData(GrpcMessage data) {
    if (data is GrpcData) {
      if (!_headers.isCompleted) {
        _responseError(const GrpcError.unimplemented('Received data before headers'));
        return;
      }
      if (_trailers.isCompleted) {
        _responseError(const GrpcError.unimplemented('Received data after trailers'));
        return;
      }
      try {
        ...
        _responseError(const GrpcError.dataLoss('Error parsing response'), s);
      }
      ...
      if (_trailers.isCompleted) {
        _responseError(const GrpcError.unimplemented('Received multiple trailers'));
        return;
      }
      ...
      _responseError(const GrpcError.unimplemented('Unexpected frame received'));
    }
  }
  ...
  void _onResponseDone() {
    if (!_headers.isCompleted) {
      _responseError(const GrpcError.unavailable('Did not receive anything'));
      return;
    }
    if (!_trailers.isCompleted) {
      if (_hasReceivedResponses) {
        // Trailers are required after receiving data.
        _responseError(const GrpcError.unavailable('Missing trailers'));
        return;
      }
  ...
  Future<void> cancel() {
    if (!_responses.isClosed) {
      const error = GrpcError.cancelled('Cancelled by client.');
