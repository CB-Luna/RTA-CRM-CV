...
class ConfigPageProvider extends ChangeNotifier {
  ...
  Future<bool> createMenuPage(String valor) async {
    try {
      ...
      // Menu
      final resMenu = await supabase
          .from('menu_dashboards')
          .select("menu_name, category_fk")
          .eq('category_fk', 1);
      ...
      //Option
      final resOption = await supabase
          .from('menu_dashboards')
          .select("menu_name, category_fk")
          .eq('category_fk', 3);
      ...
      //SubMenu
      final resSubMenu = await supabase
          .from('menu_dashboards')
          .select("menu_name, category_fk")
          .eq('category_fk', 2);
      ...
      print(
          "Desde el provider, el controllerName: ${controllerName.text} y controllerRoute: ${controllerRoute.text}");
      // print(res);
  ...
  Future<bool> deleteMenuPage() async {
    try {
      await supabase
          .from('menu_dashboards')
          .delete()
          .eq('menu_name', controllerName.text);
      notifyListeners();
